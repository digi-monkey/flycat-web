...
export interface LightningPayReq {
  encode(
    inputData: PaymentRequestObject,
    addDefaults?: boolean,
  ...
  sign(
    inputPayReqObj: PaymentRequestObject,
    inputPrivateKey: string | Buffer,
  ): PaymentRequestObject;
  satToHrp(satoshis: number | string): string;
  millisatToHrp(millisatoshis: number | string): string;
  hrpToSat(hrpString: string, outputString?: boolean): string | BN;
  hrpToMillisat(hrpString: string, outputString?: boolean): string | BN;
}
