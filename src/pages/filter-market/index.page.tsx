...
import { useFilterNoscript } from './hook/useFilterNoscripts';
...
export function FilterMarket() {
  ...
  const filterNoscripts = useFilterNoscript({ worker });
    ...
    <BaseLayout>
      <Left>
        ...
        <div className="flex flex-wrap flex-start gap-0.5 space-y-1 px-6">
          {filterNoscripts.map(noscript => {
            const disabled = filterOtpSetting.isAdded(noscript.event);
            return (
              <div
                className="flex flex-col justify-between rounded-lg border border-gray-300 border-solid w-[240px] px-4 py-2 bg-neutral-100"
                key={noscript.event.id}
              >
                <div className="flex justify-center flex-col text-center">
                  <div className="font-poppins text-lg">{noscript.title}</div>
                </div>
                ...
                <div className="w-full flex justify-between mt-1">
                  <div className="flex justify-center items-center gap-0.5">
                    <Avatar.Root className="flex justify-center items-center w-8 h-8 bg-gray-200 rounded-full overflow-hidden m-auto">
                      <Avatar.Image
                        src={noscript?.picture}
                        alt={noscript.title}
                        className="w-full h-full"
                        sizes=""
                      />
                      <Avatar.Fallback className="text-lg font-medium uppercase text-gray-400">
                        {noscript.event.pubkey.slice(0, 2)}
                      </Avatar.Fallback>
                    </Avatar.Root>
                    <div className="text-sm text-gray-400">
                      {noscript.event.pubkey.slice(0, 7)}
                    </div>
                  </div>
                  <button
                    disabled={disabled}
                    ...
                    onClick={() => filterOtpSetting.addOpt(noscript.event)}
                  >
                    <Icon className="w-6 h-6" type="icon-plus" />
                    {filterOtpSetting.isAdded(noscript.event)
                      ? 'Already added'
                      : 'add'}
                  </button>
                </div>
        ...
        <div className="flex flex-wrap flex-start gap-0.5 space-y-1 px-6">
          {filterOtpSetting.getOpts().map(opt => (
            ...
            >
              <div>
                {opt.title}@{opt.event.pubkey.slice(0, 7)}
              </div>
              <button
                className="rounded border-gray-300 border border-solid cursor-pointer"
                onClick={() => filterOtpSetting.deleteOpt(opt.event)}
              >
                delete
              </button>
            </div>
